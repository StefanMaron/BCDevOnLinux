...
echo -e "${GREEN}Building Business Central with Wine...${NC}"
...
# Build command
BUILD_CMD="docker compose -f compose.yml build"
...
if [ $? -eq 0 ]; then
    ...
    if [ "$BUILD_ONLY" = false ]; then
        ...
        echo ""
        echo "To view logs:"
        echo "  docker compose -f compose.yml logs -f bc"
        echo ""
        echo "To check Wine version:"
        echo "  docker compose -f compose.yml exec bc wine --version"
    fi
else
